...
const NoteTaskAppWithRouter = () => { 
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        const loadedPrefs = { 
            ...
            customGeminiApiKey: parsedData.userPreferences?.customGeminiApiKey || initialUserPreferences.customGeminiApiKey,
            selectedAiModel: parsedData.userPreferences?.selectedAiModel || initialUserPreferences.selectedAiModel,
            apiKeyMode: parsedData.userPreferences?.apiKeyMode || initialUserPreferences.apiKeyMode,
        };
  ...
  useEffect(() => {
    try {
      ...
      // This check prevents initialization with dummy values if the user hasn't updated them.
      if (firebaseConfig.apiKey === "YOUR_API_KEY_HERE" || 
        ...
        console.warn(
          ...
          "Please update firebaseConfig in NoteTaskApp.tsx (around line 40) " +
        ...
        // Optionally, do not initialize if placeholders are detected, or let it fail gracefully.
        // For now, we'll prevent initialization to avoid SDK errors with bad configs.
        return; 
      }
      ...
      console.log("Firebase initialized successfully with provided configuration.", { app, auth, db, storage });
    } catch (error) {
      ...
      // or guide the user to check their Firebase configuration.
    }
 ...
 useEffect(() => { const htmlElement = document.documentElement; if (themes[activeThemeKey]?.name.toLowerCase().includes('dark') || themes[activeThemeKey]?.name.toLowerCase().includes('deep') || themes[activeThemeKey]?.name.toLowerCase().includes('night')) htmlElement.classList.add('dark'); else htmlElement.classList.remove('dark'); if (document.body && userPreferences.selectedFontFamily) document.body.style.fontFamily = userPreferences.selectedFontFamily; }, [activeThemeKey, userPreferences.selectedFontFamily]);
  ...
  const handleAiDecomposeTaskRequest = async (task: AppTask) => { 
    ...
    let apiKeyForCall = userPreferences.customGeminiApiKey;
    if (userPreferences.apiKeyMode === 'prompt') {
      try {
    ...
    } else if (userPreferences.apiKeyMode === 'server-default') {
      apiKeyForCall = undefined;
    }

    try { 
        const subtasksFromAi = await generateSubtasksForTask(task.title, task.category, apiKeyForCall, userPreferences.selectedAiModel); 
  ...
  const handleSubmitAi = async (selectedContextLoreIds?: string[]) => {
    ...
    try {
      ...
      for await (const chunk of generateAiContentStream(systemInstructionToUse, fullUserPrompt, chatHistoryAi, apiKeyForCall, modelToUse)) {
